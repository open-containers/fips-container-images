...
RUN cd Python-${PYTHON_VERSION} \
    ...
    && ldconfig -v \
    && ln -s -f /opt/python-fips/bin/python3.10 /usr/bin/python3 \
    && ln -s -f /opt/python-fips/bin/python3.10 /usr/local/bin/python3 \
    && ln -s -f /opt/python-fips/bin/python3.10 /usr/bin/python
...
RUN /opt/python-fips/bin/python3 -m ensurepip \
    ...
    && ln -s -f /opt/python-fips/bin/pip3.10 /opt/python-fips/bin/pip \
    && rm -rf Python-${PYTHON_VERSION} \
    && find /opt/python-fips -depth \
       \( -name 'test' -o -name 'tests' -o -name '*.pyc' -o -name '*.pyo' -o -name 'idlelib' -o -name 'tkinter' \) \
       -exec rm -rf {} + \
    && strip --strip-unneeded /usr/local/ssl/bin/openssl /opt/python-fips/bin/python3.10
